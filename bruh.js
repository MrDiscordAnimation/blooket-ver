...
io.on('connection', socket => {
  ...
  socket.on('host', (host) => {
    if (host[0] === "join"){
      ...
      fs.readFile("code/"+host[1]+'.json', (err, dtA) => {
        if (err){
        console.log(dtA);
    ...
    if (host[0] === "ban"){
      fs.unlink("code/"+host[1]+".json", (err) => {
        if (err) {
          console.error(err);
          return io.emit('server', { status: 403, respond: "ban", code: host[1] });
        }
        //file removed
        io.emit('server', { status: 403, respond: "ban", code: host[1] });
      });
    }
  })
